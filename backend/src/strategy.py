...
class ThresholdStrategy(bt.Strategy):
    params = (
        ...
        ("strategy_name", "constantPrice")
    )

    def __init__(self):
        # initializes memory table
        self.trades = []
        self.strategyName = self.params.strategy_name
        #self.metrics = []
...
def backtest(buy, sell, start, end, ticker, strategy):
    ...
    cerebro.addstrategy(ThresholdStrategy, buy_threshold=buy, sell_threshold=sell, strategy_name=strategy)
